...
function app(transport){
  const nodes=new Map(),calls=[],events={};let interval,prints=0;
  class E{
  ...
  const context={document,GuestAdminCore:core,WeddingCore:wedding,crypto:webcrypto,AbortController,Intl,Date,setTimeout,clearTimeout,setInterval:f=>{interval=f;},addEventListener:(n,f)=>{events[n]=f;},print:()=>{prints++;},
  ...
  return {$,event,calls,interval,events,prints:()=>prints,login:async()=>{$('access-key').value='b'.repeat(64);await event('access-form','submit');}};
}
